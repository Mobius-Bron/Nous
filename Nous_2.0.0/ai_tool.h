...
__global__ void matrixMultiply(float *A, float *B, float *C, int M, int N, int K) { // 有问题，要重写
    int Row = blockIdx.y * blockDim.y + threadIdx.y;
    int Col = blockIdx.x * blockDim.x + threadIdx.x;

    float Cvalue = 0.0f;

    // Accumulate partial sums for each element in C
    for (int k = 0; k < N; ++k) {
        float Avalue = A[Row * N + k];
        float Bvalue = B[k * K + Col];
        Cvalue += Avalue * Bvalue;
    }

    if (Row < M && Col < K) {
        atomicAdd(&C[Row * K + Col], Cvalue);
    }
}
...
void initRandomFloatData(float* addr, int elemCount) {
    for (int i = 0; i < elemCount; i++) {
        addr[i] = static_cast<float>(rand() & 0xFF) / 100.f;
    }
}
...
void Linear::forward(float* x, float* output) {
    ...
    int K = output_features_;  // 输入通道数（与N相同

    // 线程块大小和网格大小
    dim3 threadsPerBlock(10, 10); // 每个块的线程数
